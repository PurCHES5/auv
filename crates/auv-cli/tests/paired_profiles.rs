...
async fn paired_profile_uses_short_token_bearer_without_pki() {
  ...
  let target_device_id = "device_018f1f00-0000-7000-8000-000000000001";
  fs::write(control_root.join("device-id"), format!("{target_device_id}\n")).unwrap();

  let pairing_path = directory.path().join("pairings.json");
  let pairing = PairingStore::open(pairing_path.clone()).expect("open pairing store");
  let token = pairing.issue_token(None).expect("issue bootstrap token").expose_once();
  drop(pairing);
