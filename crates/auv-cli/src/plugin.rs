...
use auv::AuvContext;
use auv_api_proto::auv::api::daemon::v1::Device;
...
impl ResolvedExecutionContext {
  pub(crate) async fn finish(self, succeeded: bool) -> Result<(), String> {
    ...
    let outcome = if succeeded {
      auv_api_proto::auv::api::daemon::v1::RunOutcome::Succeeded
    } else {
      auv_api_proto::auv::api::daemon::v1::RunOutcome::Failed
    };
    let client = auv::Client::from_context(self.context).await.map_err(|error| error.to_string())?;
    client.grpc().runs().stop_run(run_id, outcome).await.map_err(|status| format!("failed to stop the implicit Run: {status}"))?;
    Ok(())
  }
}
...
async fn resolve_selected_context(
  ...
  if create_implicit_run {
    let run = client
      .run(auv::client::RunOptions {
        selection: parent.run_id.clone().map(auv::client::RunSelection::Existing).unwrap_or_default(),
        device: auv::client::DeviceSelector {
  ...
  let devices = grpc.devices().list_devices().await.map_err(|status| format!("ListDevices failed: {status}"))?;
  let run = match parent.run_id.as_deref() {
    Some(run_id) => {
      let runs = grpc.runs().list_runs().await.map_err(|status| format!("ListRuns failed while resolving Run {run_id:?}: {status}"))?;
      let canonical =
        resolve_id("Run", run_id, runs.iter().filter_map(|run| run.r#ref.as_ref().map(|reference| reference.run_id.as_str())))?;
      Some(grpc.runs().get_run(canonical).await.map_err(|status| format!("Run {run_id:?} is not available: {status}"))?)
    }
    None => None,
  };
...
fn select_device<'a>(devices: &'a [Device], parent: &ParentContextOptions) -> Result<&'a Device, String> {
  let by_id = match parent.device_id.as_deref() {
    Some(device_id) => {
      let canonical = resolve_id(
        "Device",
        device_id,
        devices.iter().filter_map(|device| device.r#ref.as_ref().map(|reference| reference.device_id.as_str())),
      )?;
      Some(
        devices
          .iter()
          .find(|device| device.r#ref.as_ref().is_some_and(|reference| reference.device_id == canonical))
          .expect("resolved Device came from this collection"),
      )
    }
    None => None,
  };
  ...
  match (by_id, by_name) {
    (Some(by_id), Some(by_name)) if !std::ptr::eq(by_id, by_name) => {
      let by_name_id = by_name.r#ref.as_ref().map(|reference| reference.device_id.as_str()).unwrap_or("<missing>");
      let by_id_id = by_id.r#ref.as_ref().map(|reference| reference.device_id.as_str()).unwrap_or("<missing>");
      Err(format!("--device and --device-id select different Devices ({by_name_id:?} and {by_id_id:?})"))
    }
...
fn resolve_id<'a>(kind: &str, selector: &str, candidates: impl Iterator<Item = &'a str>) -> Result<String, String> {
  let selector = compact_id(selector);
  let matches = candidates.filter(|candidate| compact_id(candidate).starts_with(&selector)).collect::<Vec<_>>();
  match matches.as_slice() {
...
fn inherited_run_device_id(run: &auv_api_proto::auv::api::daemon::v1::Run) -> Option<&str> {
