...
async fn document_write(input: InvokeCommandInput, args: DocumentWriteInvokeArgs) -> InvokeCommandResult {
  ...
  #[cfg(target_os = "macos")]
  {
    let driver = crate::MacosTextEditDriver::open_local().map_err(|error| error.to_string())?;
...
pub async fn execute_document_write<D>(
  command: DocumentWrite,
  cancellation: auv_cli_invoke::InvokeCancellation,
  mut driver: D,
...
pub fn build_invoke_output_from_report(report: &DocumentCommandReport, command: &DocumentWrite) -> InvokeCommandResult {
