...
pub struct MinecraftLiveClickOutput {
  ...
  pub verification: crate::WorldDiffVerdict,
}

#[derive(serde::Serialize)]
struct MinecraftLiveClickVerificationEvent {
  verdict: crate::WorldDiffVerdict,
...
pub async fn run_minecraft_projection_bridge(inputs: MinecraftProjectionBridgeInputs) -> AuvResult<MinecraftProjectionBridgeOutput> {
  let frame = crate::read_latest_spatial_frame_from_tail(&inputs.telemetry_sample)?
...
pub async fn run_minecraft_live_click(inputs: MinecraftLiveClickInputs) -> AuvResult<MinecraftLiveClickOutput> {
  let pre_frame = crate::read_latest_spatial_frame_from_tail(&inputs.telemetry_sample)?
    .ok_or_else(|| format!("no valid minecraft frame found in {}", inputs.telemetry_sample.display()))?;
  ...
  let window_point =
    crate::projected_window_point(&projected_point).ok_or_else(|| "projected minecraft point is not window-clickable".to_string())?;
  ...
  let post_frame =
    crate::read_latest_spatial_frame_newer_than(post_sample_path, pre_frame.monotonic_timestamp_ms, LIVE_CLICK_POST_FRAME_WAIT)?
  ...
  let world_diff_request =
    crate::verify::WorldDiffRequest::new(MinecraftBlockTarget::new(inputs.target_block)).allow_same_block_state_change();
  let verification = crate::verify::evaluate_world_diff(&pre_frame, &post_frame, &world_diff_request);
  context.in_scope(|| {
...
async fn project_capture(
  ...
  if screenshot_artifact.is_some() {
    let context = Context::current();
    super::keep_artifact_receipt(
      crate::artifact::MINECRAFT_PROJECTION_PURPOSE,
      crate::artifact::publish_minecraft_projection(Some(&context), recorded_evidence.artifact()).await,
    );
  }
