...
fn build_detection_eval_witness_quality(detection_eval_output_dir: &std::path::Path) -> AuvResult<DetectionEvalWitnessQualityOutput> {
  ...
  let quality = build_detection_eval_quality(&crate::DetectionEvalQualityInputs {
...
async fn publish_detection_eval_evidence(result: &DetectionEvalWitnessQualityOutput) {
  ...
  let _ = crate::detection_eval_witness::publish_osu_detection_eval_witness(Some(&context), &result.witness.manifest).await;
  let _ = crate::detection_eval_quality::publish_osu_detection_eval_quality(Some(&context), &result.quality.manifest).await;
}

pub async fn run_osu_visual_truth_semantic_validation(
  run_artifact_dir: PathBuf,
  output_dir: PathBuf,
) -> AuvResult<crate::VisualTruthSemanticValidationOutput> {
  let result = validate_visual_truth_semantic(VisualTruthSemanticValidationInputs {
    run_artifact_dir,
    output_dir,
  })?;
  let context = Context::current();
  let _ = crate::visual_truth_semantic::publish_osu_visual_truth_semantic(Some(&context), &result.manifest).await;
  Ok(result)
}
...
pub async fn run_osu_visual_truth_spatial_query(
  ...
  let context = Context::current();
  let _ = crate::visual_truth_spatial_query::publish_osu_visual_truth_spatial_query(Some(&context), &result.manifest).await;
  Ok(result)
}
...
async fn publish_benchmark_projection(result: &BenchmarkOutput) {
  if let Some(projection) = &result.projection {
    let context = Context::current();
    let _ = crate::projection::publish_osu_projection(Some(&context), projection).await;
  }
}
...
fn circle_size_for_wired_live_action_inputs(inputs: &QueryWiredLiveActionInputs) -> Result<f32, String> {
  ...
  let semantic: crate::VisualTruthSemanticManifest = serde_json::from_str(&semantic_json).map_err(|error| {
